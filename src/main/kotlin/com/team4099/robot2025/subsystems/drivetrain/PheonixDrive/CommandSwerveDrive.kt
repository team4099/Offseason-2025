...
class CommandSwerveDrivetrain : TunerSwerveDrivetrain, Subsystem {
    ...
    private val m_sysIdRoutineTranslation = SysIdRoutine(
        ...
        Mechanism(
            Consumer { output: Voltage? -> setControl(m_translationCharacterization.withVolts(output)) },
            null,
            this
        )
    )
    ...
    private val m_sysIdRoutineSteer = SysIdRoutine(
        SysIdRoutine.Config(
            ...
            null,  // Use default timeout (10 s)
            // Log state with SignalLogger class
            Consumer { state: SysIdRoutineLog.State? -> SignalLogger.writeString("SysIdSteer_State", state.toString()) }
        ),
        Mechanism(
    ...
    override fun periodic() {
        ...
        if (!m_hasAppliedOperatorPerspective || DriverStation.isDisabled()) {
            DriverStation.getAlliance().ifPresent(Consumer { allianceColor: Alliance? ->
    ...
    private fun startSimThread() {
        ...
        m_simNotifier = Notifier(Runnable {
