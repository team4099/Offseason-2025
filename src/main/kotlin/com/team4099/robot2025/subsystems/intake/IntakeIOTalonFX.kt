...
object IntakeIOTalonFX : IntakeIO {
    ...
    private val rollerSensor =
        ctreAngularMechanismSensor(
            rollersTalon, IntakeConstants.Rollers.GEAR_RATIO, IntakeConstants.Rollers.VOLTAGE_COMPENSATION
        )
    ...
    init {
        ...
        pivotConfiguration.Slot0.kA = pivotSensor.accelerationFeedforwardToRawUnits(IntakeConstants.PIVOT_KA)
        pivotConfiguration.Slot0.kV = pivotSensor.velocityFeedforwardToRawUnits(IntakeConstants.PIVOT_KV)
    ...
    override fun setPivotVoltage(voltage: ElectricalPotential) {
        pivotTalon.setControl(pivotVoltageControl.withOutput(voltage.inVolts))
    }

    override fun setPivotPosition(position: Angle) {
        pivotTalon.setControl(pivotPositionControl.withPosition(pivotSensor.positionToRawUnits(position)))
    }
